...
PPStep<-function(t,S,p, DIC=TRUE){ #default to including dissolved CO2 export
  with(as.list(c(S,p)),{
    ...
    LAIi = seq(0,LAIareal,length.out=50) #[m2 leaves (m ground)-2] #length.out gives ending num levs
    ...
    #--------------------------------------------------------------------------------------------------
    #PP PIPELINE (similar to GPP)
    PP_max =5 #placeholder value, in reality the constraint here should be all about DIC and CO2.
    ...
    dW2.dt = Q12-Q23-Q2-T
    dW3.dt = Q23-Q3 
    dCa.dt = LCT1*Ac + LCT2*Ac + Aa*((P+Snow)/100)*Cprecip-(Ca/V)*Qout-deltaA*Ca + PP_pot
               
    dCr.dt = alCr-Rr-Lr
    dCcwd.dt = Lw-lout
    #test equation
    dCalat.dt = LCT1*Ac - LCT2*Ac
    ...
    if(DIC==TRUE){
      ...
      return(list(c(dCw.dt,dCl.dt,dCs1.dt, dCs2.dt, dCs3.dt, dCs4.dt, dCdoc1.dt, 
                    ...
                    dCdic2.dt, dW3.dt, dCdic3.dt, dCdoc3.dt,dCalat.dt),
      ...
      return(list(c(dCw.dt,dCl.dt,dCs1.dt, dCs2.dt, dCs3.dt, dCs4.dt, dCdoc1.dt, 
                    dCdoc2.dt, dW1.dt, dW2.dt, dCa.dt,dCr.dt, dCcwd.dt, dW3.dt,dCalat.dt),
                  c(GPP=GPP,Q1=Q1, Q2=Q2, Rf = Rf, Ra = Ra, 
                    NPP = NPP, LCT1 = LCT1, Rs3=Rs3, Dwater=Dwater, 
                    Wa=Wa, TpotAreal=TpotAreal, i0=i0, T=T, 
                    Dvpd=Dvpd, Dtemp=Dtemp, Dlightbar=Dlightbar, 
